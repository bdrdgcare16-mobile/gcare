...
exports.liveEmployeeDetails = liveEmployeeDetails;
const firebase_1 = require("../config/firebase");
const geo_1 = require("../utils/geo");
function pickDate(req) {
    const q = req.query.dateIso || '';
    if (/^\d{4}-\d{2}-\d{2}$/.test(q))
        return q;
    return new Date().toISOString().slice(0, 10);
}
async function liveEmployeeDetails(req, res) {
    try {
        ...
        if (Number.isFinite(lat) &&
            ...
            geoMeters = Math.round((0, geo_1.distanceMeters)({ lat, lng }, { lat: expLat, lng: expLng }));
        }
        const status = d.checkIn && `${d.checkIn}`.trim().isNotEmpty ? 'Present' : 'Absent';
        return res.json({
            ok: true,
            data: {
                id: d.empid,
...
//# sourceMappingURL=liveEmployeeDetails.controller.js.map