...
exports.resetPassword = exports.updateProfile = exports.getProfile = exports.forgotPassword = exports.requestPasswordResetLink = exports.backfillEmployeesToUsers = exports.createEmployeeLogin = exports.changePassword = exports.getMe = exports.login = exports.register = void 0;
const bcrypt = __importStar(require("bcryptjs"));
const jwt = __importStar(require("jsonwebtoken"));
const uuid_1 = require("uuid");
const rotatePassword_1 = require("./rotatePassword");
...
const existingApp = (0, app_1.getApps)().find((a) => a.name === APP_NAME);
const adminApp = existingApp ??
    (0, app_1.initializeApp)({
        credential: SERVICE_ACCOUNT_JSON
            ? (0, app_1.cert)(JSON.parse(SERVICE_ACCOUNT_JSON))
            : (0, app_1.applicationDefault)(),
        projectId: PROJECT_ID,
    }, APP_NAME);
console.log('[ADMIN PROJECT]', PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT || process.env.GCLOUD_PROJECT);
const auth = (0, auth_1.getAuth)(adminApp);
...
const RESET_CONTINUE_URL = process.env.RESET_CONTINUE_URL || 'https://servappbackend.web.app/reset-done';
...
if (!/^AIza[0-9A-Za-z_\-]{10,}$/.test(FIREBASE_WEB_API_KEY)) {
    console.error('APP_FIREBASE_WEB_API_KEY looks invalid or missing (pattern check failed).');
}
    ...
    try {
        ...
        if (userSnap && !userSnap.empty) {
            ...
            if (!match) {
                ...
                try {
                    await (0, rotatePassword_1.rotatePassword)({
                        db,
                        userId: doc.id,
        ...
        if (mirror && !mirror.empty) {
            ...
            if (!passOK && mirrorDoc) {
                try {
                    const prev = (mirrorDoc.get('password') ||
...
const changePassword = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const tokenEmail = normEmail(req.user?.email || '');
        ...
        const prevHash = user.password || user.passwordHash || user.hashedPassword || null;
        await (0, rotatePassword_1.rotatePassword)({
            db,
            userId,
            oldHash: prevHash,
        ...
        await userDoc.ref.set({ authSource: 'local', mustChangePassword: false, updatedAt: new Date() }, { merge: true });
...
const backfillEmployeesToUsers = async (req, res) => {
    try {
...
exports.backfillEmployeesToUsers = backfillEmployeesToUsers;
...
const forgotPassword = async (req, res) => {
    if (!req.body?.email || !req.body?.newPassword) {
        return res.status(400).json({
            error: 'Provide email and newPassword or use /forgot-password/request-link.',
        });
    }
    return (0, exports.changePassword)(req, res);
};
exports.forgotPassword = forgotPassword;
// ── Simple profile endpoints (compat) ────────────────────────────────────────
const getProfile = async (req, res) => (0, exports.getMe)(req, res);
exports.getProfile = getProfile;
const updateProfile = async (req, res) => res.json({ message: 'Profile updated successfully' }); // stub or implement as needed
