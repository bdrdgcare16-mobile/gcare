...
class WorkingProfileScreen extends StatefulWidget {
  const WorkingProfileScreen({Key? key}) : super(key: key);
...
class _WorkingProfileScreenState extends State<WorkingProfileScreen> {
  ...
  Future<void> _openCameraForRegistration() async {
    ...
    try {
      print('🎯 Opening camera for face registration...');
      ...
      if (!cameraInitialized) {
        ...
        setState(() {
          _statusMessage = 'Failed to open camera. Please check permissions.';
          _isLoading = false;
        });
        return;
      }
  ...
  Future<void> _openCameraForVerification() async {
    setState(() {
      _isLoading = true;
      _statusMessage = 'Opening camera for verification...';
    });
    ...
    try {
      ...
      print('✅ Camera opened for verification');
      setState(() {
        _showCamera = true;
      ...
      print('❌ Error opening camera for verification: $e');
      setState(() {
        _statusMessage = 'Error opening camera: $e';
        _isLoading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Profile (Working)'),
                  ...
                  if (!_isFaceRegistered) ...[
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: const Text('Open Camera for Face ID'),
