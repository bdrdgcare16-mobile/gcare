// lib/features/supderadmin/super_admin_disciplinary_actions_page.dart
import 'dart:async';
...
import 'super_admin_disciplinary_detail_page.dart';

class SuperAdminDisciplinaryActionsPage extends StatefulWidget {
  const SuperAdminDisciplinaryActionsPage({super.key});

  @override
  State<SuperAdminDisciplinaryActionsPage> createState() =>
      _SuperAdminDisciplinaryActionsPageState();
}

class _SuperAdminDisciplinaryActionsPageState
    extends State<SuperAdminDisciplinaryActionsPage> {
  ...
  Future<void> _loadActions() async {
    ...
    try {
      final data = await DisciplinaryActionsService.getSuperAdminList(
  ...
  List<DisciplinaryActionModel> get _filteredActions {
    ...
    return _actions.where((a) {
      return a.employeeName.toLowerCase().contains(q) ||
          ...
          a.companyId.toLowerCase().contains(q) ||
  ...
  Widget _buildFilters() {
    final searchField = TextField(
      ...
      decoration: _inputDecoration('Search').copyWith(
        hintText: 'Name, ID, company, category, notice',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _infoRow('Created By', a.createdByName),
  ...
  Widget _buildActionButtons(DisciplinaryActionModel a) {
    ...
    final buttons = [
      viewButton,
      if (isPending) approveButton,
      if (isPending) rejectButton,
    ];
  ...
  Future<void> _approve(String id) async {
    try {
      await DisciplinaryActionsService.approve(id);
      if (!mounted) return;
  ...
  Future<void> _reject(String id) async {
    ...
    try {
      await DisciplinaryActionsService.reject(id, reason.trim());
      if (!mounted) return;
  ...
  void _openDetails(DisciplinaryActionModel a) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => SuperAdminDisciplinaryDetailPage(
          action: a,
          onStatusChanged: _loadActions,
        ),
      ),
    );
  }
          ...
          return Column(
            ...
            children: [
              ...
              if (_filteredActions.isEmpty) ...[
                ...
                const Center(
                  child: Text(
                    'No disciplinary actions awaiting review.',
