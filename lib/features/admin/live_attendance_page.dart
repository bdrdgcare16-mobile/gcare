...
class _LiveAttendancePageState extends State<LiveAttendancePage>
  ...
  Future<void> _debugFirebaseAuth() async {
    try {
      ...
      if (u == null) {
        debugPrint('[FIREBASE] No current user');
      } else {
        debugPrint('[FIREBASE] User authenticated');
      }
    } catch (e) {
      debugPrint('[FIREBASE] Auth debug error occurred');
    }
  }
  ...
  Future<void> _fetchEmployeesMeta() async {
    try {
        ...
        final List<dynamic> data = decoded is List
            ...
            : (decoded is Map<String, dynamic> && decoded['data'] is List)
                ...
                : (decoded is Map<String, dynamic> &&
                    ...
                    ? decoded['employees'] as List<dynamic>
                    : <dynamic>[];
        _metaById.clear();
        for (final e in data) {
      ...
      debugPrint('[ATTENDANCE] Employees meta error occurred');
    }
  }
  ...
  Future<void> _loadShiftsFromApi() async {
    try {
      ...
      debugPrint('[ATTENDANCE] Shifts error occurred');
    }
  }
  ...
  List<AttendanceRecord> get halfDayRecords {
    ...
    for (final r in _records) {
      if (r.status.toLowerCase().contains('half')) {
        out.add(r);
        continue;
      }
  ...
  int get lateCheckInCount {
    ...
    for (final r in _records) {
      ...
      final shiftStartDateTime = DateTime(
          now.year, now.month, now.day, shiftStart.hour, shiftStart.minute, 0);
      final shiftStartWithGrace = shiftStartDateTime
          .add(const Duration(minutes: 5)); // 5 minutes grace period
      final checkInDateTime =
          DateTime(now.year, now.month, now.day, checkInHour, checkInMinute, 0);
      ...
      final isActuallyLate =
          !isOpenShift && checkInDateTime.isAfter(shiftStartWithGrace);
                  ...
                  child: RefreshIndicator(
                    ...
                    child: CustomScrollView(
                      ...
                      slivers: [
                        ...
                        SliverToBoxAdapter(
                            child: _sectionLabel('Attendance Status')),
                        SliverToBoxAdapter(child: _statusRow()),
                        SliverToBoxAdapter(
                            child: _sectionLabel('Activity Overview')),
        ...
        child: Padding(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.wifi_off_rounded,
                size: 48, color: _C.red.withOpacity(0.5)),
                        ...
                        RichText(
                          text: const TextSpan(
                            ...
                            children: [
                              ...
                              TextSpan(
                                  text: 'Dashboard',
                                  style: TextStyle(color: _C.lv500)),
                            ],
                          ),
                        ),
                      ],
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: _C.lv500.withOpacity(0.38),
                blurRadius: 22,
  ...
  Widget _activityGrid() {
    ...
    for (final r in _records) {
      ...
      final shiftStartDateTime = DateTime(
          now.year, now.month, now.day, shiftStart.hour, shiftStart.minute, 0);
      final shiftStartWithGrace =
          shiftStartDateTime.add(const Duration(minutes: 5));
      final checkInDateTime =
          DateTime(now.year, now.month, now.day, checkInHour, checkInMinute, 0);
  ...
  Widget _aCard(_AItem it) => GestureDetector(
        onTap: it.onTap ??
            () => _showEmployeePopup(it.title.replaceAll('\n', ' '), it.list),
        child: Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
            color: Colors.white,
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        builder: (_, sc) => Container(
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: _C.lv100, borderRadius: BorderRadius.circular(20)),
                  child: Text('$count',
                      style: const TextStyle(
                          color: _C.lv600,
                          fontWeight: FontWeight.w700,
            ...
            const Divider(height: 1, color: _C.border),
            Expanded(
                child: SingleChildScrollView(controller: sc, child: child)),
          ]),
        ),
      ),
    );
  ...
  Widget _emptyState(String msg, IconData ic, Color c) => SizedBox(
        height: 180,
        child: Center(
            child:
      ...
      child: ListView.separated(
        ...
        itemBuilder: (ctx, i) {
          ...
          return _empRow(
            name: e.name,
            sub: '${e.empid}  ·  ${e.dept ?? '-'}',
            trail: _badge(e.checkIn, _C.green),
            extra: e.shiftGroup,
            onTap: () {
    ...
    try {
      ...
      _sheet(
        ...
        child: pendings.isEmpty
            ? _emptyState('No pending approvals',
      ...
      _showSimpleInfo('Failed to load: $e');
    }
  }
  ...
  void _showEmployeePopup(String title, List<AttendanceRecord> list) {
    _sheet(
      ...
      child: list.isEmpty
          ? _emptyState(
              'No employees found', Icons.person_off_outlined, _C.muted)
          : ListView.separated(
              shrinkWrap: true,
              physics: const NeverScrollableScrollPhysics(),
              padding: const EdgeInsets.fromLTRB(16, 12, 16, 16),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Info',
            style: TextStyle(color: _C.ink, fontWeight: FontWeight.w700)),
        content: Text(msg, style: const TextStyle(color: _C.muted)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('OK',
                style: TextStyle(color: _C.lv500, fontWeight: FontWeight.w600)),
          )
        ],
      ),
    );
  ...
  (TimeOfDay?, TimeOfDay?)? _parseNameRange(String s) {
    ...
    return (
      TimeOfDay(hour: _to24h(h1, p1), minute: m1),
