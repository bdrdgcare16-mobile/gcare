...
class LegendCircle extends StatelessWidget {
  ...
  const LegendCircle({required this.color, required this.label, Key? key})
      : super(key: key);
...
class StatusCard extends StatelessWidget {
  ...
  const StatusCard(this.label, this.value, this.color, {Key? key})
      : super(key: key);
...
class _MyAttendancePageState extends State<MyAttendancePage> {
  ...
  Future<void> _loadInitialAttendance() async {
    ...
    try {
      ...
      debugPrint('[MyAttendance] Initial load error: $e');

      if (!mounted) return;
      setState(() {
        _attendanceLoadError =
            'Unable to load attendance. Please go back and open this page again.';
      });
    } finally {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _loadMonth(DateTime anchor) async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        // ✅ FIX:
        // Use Sets to count each date only once.
        // This prevents high counts when the API returns duplicate records for the same date.
        final Set<String> presentDates = {};
        final Set<String> absentDates = {};
        final Set<String> leaveDates = {};
        final Set<String> lateDates = {};
        final Set<String> earlyDates = {};

        // Permission may be decimal/fractional, so store once per date instead of adding duplicates.
        final Map<String, num> permissionByDate = {};
        ...
        for (final item in data) {
          ...
          if (permissionCount > 0) {
            permissionByDate[date] = permissionCount;
          }

          final String statusText = item['status']?.toString().toLowerCase() ?? '';

          // Use API flags with correct priority order
          final bool isAbsent =
              item['isAbsent'] == true || statusText == 'absent';

          final bool isLeave =
              item['isLeave'] == true || statusText == 'leave';
          ...
          debugPrint(
            '[MyAttendance] Status determination - Date: $date, isAbsent: $isAbsent, isLeave: $isLeave, isPermission: $isPermission, isLate: $isLate, isEarly: $isEarly, hasCheckIn: $hasCheckIn, status: ${item['status']}',
          );
          ...
          // 3. Present
          //
          // Each date is counted only once using Sets.
          if (isLeave) {
            ds[date] = 'Leave';
            leaveDates.add(date);
            absentDates.remove(date);
            presentDates.remove(date);
          } else if (isAbsent) {
            ds[date] = 'Absent';
          ...
          // Permission is counted separately but not duplicated.
          if (isPermission && permissionCount > 0) {
            permissionByDate[date] = permissionCount;
          }
        }

        final int present = presentDates.length;
        final int absent = absentDates.length;
        final int leave = leaveDates.length;
