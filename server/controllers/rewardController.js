...
// //   try {
// //     const db = req.app.locals.db;
// //     const doc = await db.collection('rewards').doc(req.params.id).get();
...
// /** Normalize any incoming date value to a JS Date. Fallback = now. */
// // controllers/rewardController.js
...
//       query = query.where('empid', '==', empidParam);
//     } else {
//       // No filter => order by date desc for nicer listing
//       query = query.orderBy('date', 'desc');
//     }

//     const snapshot = await query.get();
//     let rewards = snapshot.docs.map((doc) => serialize(doc));

//     // If filtered by empid, ensure newest first without requiring Firestore composite index
//     if (empidParam) {
//       rewards.sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0));
//     }
