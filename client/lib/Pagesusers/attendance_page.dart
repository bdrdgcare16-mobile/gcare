...
//       // Set the check-in time to today at the given time
//       _checkInTime = DateTime(now.year, now.month, now.day, 
//           parts[0], parts[1], parts.length > 2 ? parts[2] : 0);
...
//       // Directly open the app's location settings
//       await openAppSettings();
      
//       // Check the permission status after returning from settings
//       permission = await Geolocator.checkPermission();
      
//       // Update the UI with current permission status
//       if (mounted) {
//         setState(() {
//           _locationPermissionGranted = permission == LocationPermission.always;
//         });
        
//         // If still not granted, show a more specific message
//         if (permission != LocationPermission.always) {
//           if (mounted) {
//             ScaffoldMessenger.of(context).showSnackBar(
//               const SnackBar(
//                 content: Text('Please enable "Allow all the time" for location access'),
...
//       _checkInTime = now; // Set check-in time
//       _checkOutTime = null; // Reset check-out time
//     });
//     _startWorkTimer();
//     await _saveCheckInToPrefs();
...
//         setState(() {
//           isCheckedIn = false;
//           _checkOutTime = DateTime.now(); // Set check-out time
...
//               left: 16,
//               top: 16,  // Reset to original top padding
//               right: 16,
...
//                                 style: TextStyle(
//                                   fontSize: 14,
//                                   color: _checkOutTime != null ? Colors.red : Colors.grey[600],
//                                 ),
//                               ),
//                             ],
//                           ),
...
//                   ),
//                 ),
//                 const SizedBox(height: 40),  // Increased from 20 to 40 to add space after employee info
//                 SizedBox(
//                   width: 96,
//                   height: 96,
//                   child: ClipRRect(
//                     borderRadius: BorderRadius.circular(40),
//                     child: Image.asset('assets/images/splash2.jpg', fit: BoxFit.contain),
//                   ),
//                 ),
...
class _AttendanceScreenState extends State<AttendanceScreen>
  ...
  // ✅ Enable auto-checkout in client
  final bool _autoCheckoutEnabled = true;
  Timer? _autoCheckoutTimer;
  ...
  Future<void> _loadUserInfo() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        // 🔹 Load dynamic shift times from Firestore once we know selectedShift
  ...
  bool _isOpenShift(String s) {
    ...
    return name.contains('open'); // e.g., "Open", "Open Shift"
  }
  ...
  /// Query backend for today's overtime hours (duration in HOURS).
  Future<int> _getTodayOvertimeHours({required String empid}) async {
    try {
      final ymd = _todayYmd();
      final url = Uri.parse('$_apiBase/overtime?empid=$empid&date=$ymd');
      final res = await http.get(url, headers: {'Authorization': 'Bearer ${CompanyData.token}'});
      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
        final v = body['overtimeHours'];
  ...
  String? _detectCheckInCategory() {
    ...
    // ✅ 5-minute grace after start; after that => Late Check-in
  ...
  /// ✅ Checkout rules:
  /// - Before end => "Early Checkout"
  /// - Between [end, end+5min] => allowed (no prompt)
  /// - After end+5min => "Late Checkout"
  String? _detectCheckoutCategory() {
    if (_isOpenShift(selectedShift)) return null;
    ...
    // within 5-minute grace window => no reason required
    return null;
  }

  /// Fetch ALL reasons from /api/reasons
  Future<List<Map<String, String>>> _fetchAllReasons() async {
    try {
      final res = await http.get(Uri.parse('$_apiBase/reasons?limit=200'));
      if (res.statusCode != 200) return <Map<String, String>>[];
  ...
  // 🔹 Load shift times from Firestore once and cache them
  Future<void> _loadShiftTimes() async {
    try {
      ...
      final endStr   = (data['endTime']   ?? '').toString().trim(); // "16:45"

      TimeOfDay? start = _parseHHmm(startStr);
      TimeOfDay? end   = _parseHHmm(endStr);

      if (start == null || end == null) {
        // Optional: try parse from "name" like "8.45 AM - 4.45 PM" if present
  ...
  // 🔹 Use Firestore-loaded times if available; otherwise safe fallback 09:00–18:00
  ShiftTimes _getShiftTimes(String shift) {
    if (_shiftTimes != null) return _shiftTimes!;
    return const ShiftTimes(TimeOfDay(hour: 9, minute: 0), TimeOfDay(hour: 18, minute: 0));
  }

  // 🔹 Helpers to parse "HH:mm" and optional "h.mm AM - h.mm PM"
  ...
  ShiftTimes? _parseNameRange(String s) {
    ...
    return ShiftTimes(TimeOfDay(hour: h1, minute: m1), TimeOfDay(hour: h2, minute: m2));
  }
  ...
  void _scheduleAutoCheckout() {
    ...
    final now = DateTime.now();
    final initialFire = endDT.isAfter(now) ? endDT : now.add(const Duration(seconds: 1));
    final initialDelay = initialFire.difference(now);

    _autoCheckoutTimer = Timer(initialDelay, () async {
      if (!mounted || !isCheckedIn) return;

      // 1) Wait 30 minutes after shift end
      await Future.delayed(const Duration(minutes: 30));
      if (!mounted || !isCheckedIn) return;

      // 2) Extend by overtime hours (if any)
      final otHours = await _getTodayOvertimeHours(empid: userId);
      if (otHours > 0) {
        await Future.delayed(Duration(hours: otHours));
        if (!mounted || !isCheckedIn) return;
      }

      // 3) Auto-checkout silently
      await _performCheckOut(silent: true);
    });
  }
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(Icons.calendar_today, size: 14, color: Colors.grey[600]),
                          const SizedBox(width: 6),
                          Text('${DateTime.now().day.toString().padLeft(2, '0')} ${_getMonthName(DateTime.now().month)} ${DateTime.now().year}'),
                        ],
                      ),
                    ],
                  ),
