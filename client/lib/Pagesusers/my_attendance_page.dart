...
//   }
// }
// lib/Pagesusers/my_attendance_page.dart
import 'dart:convert';
import 'dart:html' as html; // Flutter Web storage
import 'package:flutter/material.dart';
...
class _MyAttendancePageState extends State<MyAttendancePage> {
  ...
  void _persistTokenIfPresent() {
    try {
      ...
      if (t != null && t.isNotEmpty) {
        ...
        debugPrint(
          '[Attendance] persisted token from CompanyData (${t.length} chars)',
        );
      }
    } catch (_) {
      // ignore
  ...
  Future<void> _bootstrap() async {
    ...
    debugPrint('[Attendance] bootstrap empid=$_empid');
  }
  ...
  String? _tryEmpIdFromModel(AttendanceData d) {
    try {
      ...
      if (v != null && v.isNotEmpty) {
        debugPrint('[Attendance] empid from model: $v');
        return v;
      }
    } catch (_) {}
    return null;
  ...
  String? _tryEmpIdFromLocalStorage() {
      ...
      try {
        ...
        if (me is Map) {
          ...
          if ((me['empid'] ?? '').toString().isNotEmpty) {
            ...
            debugPrint('[Attendance] empid from localStorage["me"]: $v');
            return v;
          }
          if (ep is Map && (ep['empid'] ?? '').toString().isNotEmpty) {
            final v = ep['empid'].toString();
            debugPrint(
              '[Attendance] empid from localStorage["me"].employeeProfile: $v',
            );
            return v;
          }
        }
    ...
    for (final k in keys) {
      ...
      if (v != null && v.isNotEmpty) {
        debugPrint('[Attendance] empid from localStorage["$k"]: $v');
        return v;
      }
    }
    return null;
  ...
  Future<String?> _fetchEmpIdFromAuthMe() async {
    ...
    if (token == null) {
      debugPrint('[Attendance] no token for /api/auth/me');
      return null;
    }
    try {
      final uri = Uri.parse('$apiBase/api/auth/me');
      debugPrint('[Attendance] GET $uri');
      ...
      debugPrint('[Attendance] /api/auth/me status=${resp.statusCode}');
      ...
      debugPrint('[Attendance] /api/auth/me error: $e');
    }
    return null;
  }

  // Robust token getter (looks for any JWT-like string)
  String? _getToken() {
    // 1) Common keys
    ...
    // 2) Scan localStorage for any JWT-looking value
    try {
  ...
  Future<void> _loadMonth(DateTime anchor) async {
    if (_empid == null || _empid!.isEmpty) {
      debugPrint('[Attendance] missing empid; using widget.data fallback');
      setState(() {
    ...
    final token = _getToken();
    final uri = Uri.parse('$apiBase/api/attendance/month-view/$_empid/$y/$m');
    debugPrint('[Attendance] GET $uri token? ${token != null}');
    ...
    try {
      ...
      debugPrint(
        '[Attendance] status=${resp.statusCode} bodyLen=${resp.body.length}',
      );
      if (resp.statusCode == 200) {
        ...
        debugPrint(
          '[Attendance] dayStatuses=${ds.length} first=${ds.keys.take(3).toList()}',
        );
        ...
        setState(() {
          ...
          _holiday = totals['holiday'] ?? 0;
          _weekOff = totals['weekOff'] ?? 0;
        ...
        debugPrint('[Attendance] ERR ${resp.statusCode}: ${resp.body}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Month-view ${resp.statusCode}: ${resp.body}'),
          ),
        );
        setState(() {
          _present = widget.data.presentCount;
  ...
  Color? _colorForDay(DateTime day) {
    ...
    switch (status) {
      ...
      default:
        // Fallback to old arrays if API didn’t give a map
  ...
  Widget _buildCalendar() {
    return Container(
      ...
      child: TableCalendar(
        ...
        firstDay: DateTime.utc(2023, 1, 1),
        lastDay: DateTime.utc(2030, 12, 31),
        rowHeight: 44, // give our badge room to render
        selectedDayPredicate: (day) => isSameDay(_selectedDay, day),
        onDaySelected: (selectedDay, focusedDay) {
          setState(() {
            _selectedDay = selectedDay;
        ...
        calendarStyle: const CalendarStyle(
          weekendTextStyle: TextStyle(color: Colors.red),
          outsideDaysVisible: false,
          isTodayHighlighted: false, // we render today explicitly
        ),
        headerStyle: const HeaderStyle(
          formatButtonVisible: false,
          titleCentered: true,
  ...
  /// A robust cell that always paints a circular badge sized to the cell.
  Widget _dayCell(
    DateTime day, {
    Color? color,
      ...
      builder: (context, constraints) {
        ...
        return Center(
          child: Container(
            ...
            height: dia,
            decoration: BoxDecoration(
              color: hasBg ? bg.withOpacity(0.90) : Colors.transparent,
              shape: BoxShape.circle,
            ),
            alignment: Alignment.center,
            child: Text(
